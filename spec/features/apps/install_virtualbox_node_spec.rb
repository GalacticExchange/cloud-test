...
# * node should be installed on the client machine
  ...
  describe 'install app' do
    ...
    it "install app on node" do
      ...
      app_base_dir = "/home/#{@sys_user}/.gex/vagrant/applications/#{@appname}/"
      ...
      # install container
      cmd = %Q{cd /home/#{@sys_user}/.gex/vagrant && /bin/bash -c "vagrant --image-file=applications/#{@appname}/#{@appname}.tar.gz --app-name=#{@appname} -- provision --provision-with install_container 2>&1"}
      ...
      # check
      cmd = %Q{cd /home/#{@sys_user}/.gex/vagrant && vagrant ssh -- "docker images | grep #{@appname}" }
      ...
      cmd = %Q{cd /home/#{@sys_user}/.gex/vagrant && vagrant ssh -- "docker rm -f #{@appname}" }
      res = run_cmd_on_client(@client_config, cmd)

      #
      cmd_vagrant = %Q{vagrant --app-name=#{@appname} --json-file=applications/#{@appname}/config.json -- provision --provision-with run_container}
      cmd = %Q{cd /home/#{@sys_user}/.gex/vagrant && /bin/bash -c "#{cmd_vagrant} 2>&1"}
