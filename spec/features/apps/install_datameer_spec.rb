RSpec.describe "Testing install app", :type => :request do
  ...
  describe "apps installation" do
    ...
    it 'datameer installation'  do
      ...
      vagrant_status = wait_log_event("vagrant_status", 600, {cluster_id: cluster_id})
      expect(vagrant_status).not_to be_nil
      puts "Vagrant status --->  command completed"

      application_install = wait_log_event("application_install", 420, {cluster_id: cluster_id})
      ...
      application_run = wait_log_event("application_run", 120, {cluster_id: cluster_id})
