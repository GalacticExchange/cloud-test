module GexcloudHelpers
  ...
  def self.get_server_config_node_local
    ...
    # get node ip
    # run command locally
    #res = `(cd $HOME/.gex/vagrant && (vagrant ssh -- ifconfig eth1 | grep "inet addr" 2>&1)) 2>&1`
    res = `bash -c 'cd $HOME/.gex/vagrant && (bundle exec /usr/bin/vagrant ssh -- ifconfig eth1 | grep "inet addr")'`
